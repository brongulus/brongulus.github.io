...
If the most significant bitwise addition generates a carry of 1, /overflow/ occurs, by ignoring it one achieves /n/ correct bits for adding two /n/-bit numbers.
**** Signed Numbers
...
**** RAM
A RAM chip consists of /n/ 16-bit register chips which can be selected and manipulated separately. By specifying a particular address (0 to n-1), a particular register can be selected for read/write operations. Note that the access time of any randomly selected register is insantaneous.
